...
Модели хранятся в отдельном каталоге `MODELS_PATH` (по умолчанию `./models`), история и результаты — в `DATA_PATH` (`./data`). Оба лежат на хосте, поэтому пересборка и обновление образа их не трогают. Чтобы держать модели на другом диске, поменяйте `MODELS_PATH` в `.env`.
