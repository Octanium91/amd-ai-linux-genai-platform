...
- **Prompt assistant (optional):** a "To prompt" button next to the prompt. A language model on an [Ollama](https://ollama.com) server (dolphin-phi or dolphin-llama3 recommended) turns a short idea in any language into a detailed English prompt written for the selected mode: comma-separated tags within CLIP's 75 tokens for Stable Diffusion 1.5, flowing sentences with motion and camera for Wan. Connected in Settings; the button is active only while the model is available. See [docs/models.md](docs/models.md#prompt-assistant).
