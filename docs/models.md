...
Скачивать и удалять модели может только администратор. При первом запуске, пока нет ни одного готового режима, администратор видит экран выбора наборов моделей ([catalog/packs.json](../catalog/packs.json)): обязательная основа, рекомендуемые под это железо наборы, размеры, свободное место на диске.
...
Обе экстра-настройки в интерфейсе красные: работают, но с оговорками.

- **Экстра-качество:** вдвое больше шагов, чем «Высокое» (или `defaults.quality.extra`, если режим задаёт своё значение). Время удваивается, а прирост качества уже небольшой.
- **Экстра-длительность:** правая половина ползунка, от предела модели до двойного (AnimateDiff/AnimateLCM — 4 → 8 с, Wan 2.2 — 5 → 10 с). Модели не генерируют длиннее за один проход, поэтому видео собирается из **двух сегментов**:
  1. сначала генерируется первый сегмент;
  2. его последний кадр становится стартовой картинкой второго (как в режиме «картинка → видео»);
  3. ffmpeg склеивает сегменты, выбрасывая дублирующийся кадр на стыке, и интерполирует FPS.

  Время удваивается, на стыке возможен скачок движения или смена деталей. Доступно только режимам, которые умеют «картинка → видео» (у Wan 2.1 1.3B нет).

## Стартовые шаблоны

[catalog/templates.json](../catalog/templates.json) содержит по 10 шаблонов для изображений и видео. При открытии раздела форма заполняется случайным шаблоном, причём предпочтение отдаётся режимам, модели которых уже скачаны. Выбрать шаблон в интерфейсе нельзя: это стартовые примеры, которые показывают, как писать промпты и какие параметры дают лучший результат.

- **Изображения** — по рекомендациям автора Realistic Vision 6:
  - промпт вида «RAW photo, …, 8k uhd, dslr, soft lighting, film grain»;
  - авторский негативный промпт;
  - `dpm++2m` + `karras`, CFG 5–6, «Высокое» (40 шагов);
  - крупные разрешения 768×1024, 896×896, 1024×768, 1152×640.
- **Видео** — по эталонным конфигам AnimateDiff для Realistic Vision, адаптированным под AnimateLCM:
  - в промпте явные признаки движения: wind, waves, flowing, camera tracking;
  - 16 кадров (2 с), «Высокое» (8 шагов);
  - CFG 1.5 — контрастнее и больше движения, чем CFG 1.

Шаблоны можно дополнять: поле `negative` ссылается на именованный негатив из `negatives`, недостающие параметры берутся из `defaults`.
